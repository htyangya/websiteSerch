...
class CmsFile(db.Model):
    ...
    def setCtxIndexedFlg(self, file_id, ctx_indexed_flg):
        updFile = db.session.query(CmsFile).filter(
            CmsFile.file_id == file_id,
            CmsFile.is_deleted == 0).first()
        updFile.ctx_indexed_flg = ctx_indexed_flg

    def getFile(self, file_id):
        return db.session.query(CmsFile).filter(CmsFile.file_id == file_id).first()

    def getFileList(self, parent_object_id, file_type_id):
        return db.session.query(CmsFile).filter(
            CmsFile.parent_object_id == parent_object_id,
            CmsFile.file_type_id == file_type_id,
            CmsFile.is_deleted == 0).all()

    def addFile(self, cmsFile):
        return db.session.add(cmsFile)

    def delFile(self, fileId, userId):
        del_file = db.session.query(CmsFile).filter(CmsFile.file_id == fileId).first()
        del_file.is_deleted = 1
        del_file.deleted_at = datetime.now()
        del_file.deleted_by = userId

    def delFiles(self, parent_object_id, userId):
        del_file_list = db.session.query(CmsFile).filter(CmsFile.parent_object_id == parent_object_id).all()
    ...
    def get_file_list(self, object_id, file_type_id=None):
        ...
        selectSql = selectSql.format(select_columns=CmsFile.SELECT_COLUMN_STR)
        t = text(selectSql)
