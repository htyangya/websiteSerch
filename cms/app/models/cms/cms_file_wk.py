...
class CmsFileWk(db.Model):
    ...
    def getFile(self, edit_id, file_id):
        return db.session.query(CmsFileWk).filter(CmsFileWk.edit_id == edit_id,
                                                  CmsFileWk.file_id == file_id,
                                                  CmsFileWk.is_deleted == 0).first()

    def getFileList(self, edit_id, parent_object_id, file_type_id=None):
        query = db.session.query(CmsFileWk).filter(CmsFileWk.edit_id == edit_id,
                                                   CmsFileWk.parent_object_id == parent_object_id)
        if file_type_id != None and file_type_id > 0:
            query = query.filter(CmsFileWk.file_type_id == file_type_id)
        return query.all()

    def addFile(self, cmsFile, userId):
        cmsFile.is_deleted = 0
        cmsFile.created_at = datetime.now()
        cmsFile.created_by = userId
        cmsFile.updated_at = datetime.now()
        cmsFile.updated_by = userId
        return db.session.add(cmsFile)

    def delFile(self, edit_id, file_id, userId):
        del_file = db.session.query(CmsFileWk).filter(CmsFileWk.edit_id == edit_id,
                                                      CmsFileWk.file_id == file_id).first()
    ...
    def delFiles(self, parent_object_id, file_type_id=None):
        query = db.session.query(CmsFileWk).filter(CmsFileWk.parent_object_id == parent_object_id)
        if file_type_id != None and file_type_id > 0:
            query = query.filter(CmsFileWk.file_type_id == file_type_id)
        query.delete()
    ...
    def get_file_list(self, edit_id, object_id, file_type_id=None):
        ...
        selectSql = selectSql.format(select_columns=CmsFileWk.SELECT_COLUMN_STR)
        t = text(selectSql)
    ...
    def chkExistsFile(self, af_obj):
        cmsFileE = CmsFileWk()
        file = cmsFileE.getExistsFile(af_obj)
        if not file:
            return False
