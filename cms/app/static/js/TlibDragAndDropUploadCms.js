...
function reload_edit_file_list(result, fileTypeId, files) {
	...
	var fd = create_form_data(func, files, fileTypeId);
...
function post_file_upload(result, fileTypeId, files) {
	hide_loading_msg();
	reload_edit_file_list(result, fileTypeId, files);
}

function post_file_upload_by_drop(xhr, fileTypeId, files) {
	if(xhr.status !== 200) {
		hide_loading_msg();
	...
	post_file_upload(result, fileTypeId, files);
}
...
function do_file_upload_by_drop(fileTypeId, files) {
	var err_func_name = 'do_file_upload_by_drop';
	var progress = document.getElementById('tlibFileUploadProgressBar');

	var fd = create_form_data(args['func']['file_upload'], files, fileTypeId);
...
function post_check_overwrite_by_drop(result, fileTypeId, files) {
	if(result.status === 'Error') {
		hide_loading_msg();
		show_error_message(result.msg);
		return;
	...
	if(result.check_overwrite_flg) {
		...
		do_file_upload_by_drop(fileTypeId, files);
	}
}

function create_form_data(func, files, fileTypeId) {
	// FormDataオブジェクトを用意
	var fd = new FormData();
	fd.append("func", func);
...
function init_drop_file_target() {
	...
	var checkOverwrite = function(fileTypeId, files) {
		// FormDataオブジェクトを用意
		var fd = create_form_data(args['func']['check_overwrite'], files, fileTypeId);
		...
		$.ajax({
			...
			contentType: false,
			success: function(result) {
					post_check_overwrite_by_drop(result, fileTypeId, files);
				},
			error: function(xmlhttprequest, textstatus, errorThrown) {
				hide_loading_msg();
	...
	var checkOverwriteLocal = function(fileTypeId, files, file_list_for_check) {
		var i;
	...
	var uploadFiles = function(fileTypeId, files) {
		if(args['func']['check_overwrite'] !== undefined) {
			checkOverwrite(fileTypeId, files);
			return;
		}
		if(args['file_list_for_check_overwrite'] !== undefined) {
			setTimeout(function() {
				checkOverwriteLocal(fileTypeId, files,
	...
	var confirmUploadFiles = function(fileTypeId, files, chk_flg) {
		...
		if(chk_flg) {
			confirm_2cb_noclose(fileNameList + '<br><br>' +
				...
				function() { uploadFiles(fileTypeId, files); },
				function() { hide_loading_msg(); });
		} else {
			uploadFiles(fileTypeId, files);
		}
	};
	...
	var onDropFile = function(evt) {
		...
		confirmUploadFiles(fileTypeId, files, true);
	};
	...
	$("." + args['elm_ids']['input_class']).change(function(){
		...
		confirmUploadFiles(fileTypeId, files, false);
	});
}
