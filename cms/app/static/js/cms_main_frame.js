...
var cms_main_frame = function(view_type) {

var jstree_select_flg = false;
var last_list_hash = '';
var cur_tree_type = '';
var cur_contents_id = '';
...
function post_list_data(x, s) {
    ...
    var folder_msg = result.folder_information_message;
    if (typeof(folder_msg) !== "undefined" && folder_msg != null && folder_msg!== "") {
        $('#folder_msg_div').html(folder_msg);
        $('#folder_msg_div').show();
        topH = topH + Number($('#folder_title_msg_div').height());
    } else {
        topH = 25;
    }
    if (isEditMode == 'True' && child_object_type_id !== '' && cur_view_type !== "KEYWORD") {
        topH = topH + 35;
    }
    $('#listArea').css('top', topH + 'px');
    // 提示メッセージの設定
    if (result.search_type == "COND_SEARCH") {
        $("#folder_keyword_path_div").text("検索結果 (" + fileList.length + ")");
        $("#folder_msg_div").text("");
    }
    ...
    $.each(header, function(index, value) {
        if (result.search_type == "COND_SEARCH") {
            rowEmHead.append($("<th></th>").attr("style", value.td_style).text(value.property_name));
        } else {
            aEle = $("<a></a>").attr("href", "javascript:void(0);")
                           .text(value.property_name);
            var sort_type = "";
            if(result.sort_column != null && result.sort_column == "col_" + index) {
                aEle.addClass(result.sort_type + "_w");
                sort_type = result.sort_type;
            }
            var click_func = "onSort('col_" + index +"','" + value.property_type + "', '" + sort_type + "');";
            rowEmHead.append($("<th class=\"td_link\" onclick=\"" + click_func + "\"></th>").attr("style", value.td_style).append(aEle));
        }
    ...
    $.each(fileList, function(index, entity) {
        ...
        $.each(header, function(index, value) {
            ...
            if ((typeof(cur_view_type) !== "undefined" && cur_view_type == "KEYWORD")
                 || result.search_type == "COND_SEARCH") {
            ...
            if (isEditMode == 'True') {
                ...
                if(index == 0) {
                    ...
                    var aEm = $("<a></a>").text(txtValue)
                                          ...
                                          .attr("onClick", "openPropertyPage('" + ids + "', '" + object_id + "', '" + displayOrder + "', 'NEW_WINDOW')");
...
function get_list_data() {
    ...
    var urls = "get_file_list?view_type=" + cur_view_type + "&db_id=" + $("#db_id").val() + "&id=" + id;
    if (typeof sort_column !="undefined" && sort_column !== "") {
        urls += "&sort_column=" + sort_column + "&sort_column_type=" + sort_column_type + "&sort_type=" + sort_type;
    }
	$.ajax({
		url: urls,
		cache: false,
...
function search_data_list(urls) {
    show_null_loading();
    if(document.hasFocus()) {
		$("#listArea").html("Loading...").focus();
	}
...
function on_hashchange() {
	var h = get_page_hash();
	if(h === '#!l') {
	    if (jstree_select_flg) {
	        hide_hsplitter();
		    get_list_data();
	    } else {
	        $("#view_list").jstree().deselect_all(true);
	        $('#view_list').jstree('select_node', get_page_hash_opt('id'));
	    }
	    jstree_select_flg = false;
...
function set_page_hash_reload(h, sColumn, sColumnType, sType) {
    ...
    sort_column = sColumn;
    sort_column_type = sColumnType;
    sort_type = sType;
    jstree_select_flg = true;
