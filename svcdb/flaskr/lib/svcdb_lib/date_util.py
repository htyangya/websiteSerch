...
from flaskr.lib.svcdb_lib.arr_util import ArrUtil
...
class DateUtil:
    ...
    def current_date_str():
        return DateUtil._get_date_format('%Y%m%d%H%M%S')

    @staticmethod
    def current_yyyy_date_str():
        return DateUtil._get_date_format('%Y')

    def _get_date_format(format):
        dt_now = datetime.datetime.now()
        return dt_now.strftime(format)